...
_nvtree_unpack(uint8_t **buf, const uint8_t *end) {
	...
	if (*buf + sizeof(head) > end) {
		nvtree_log("No enough size for tree header");
		return (NULL);
	}
	memcpy(&head, *buf, sizeof(head));
	if (head.magic != NVTREE_HEADER_MAGIC || head.version != NVTREE_HEADER_VERSION) {
		nvtree_log("Wrong magic or version number");
		return (NULL);
	}
	*buf += sizeof(head);
	root = nvtree_create();
...
nvtpair_unpack(uint8_t **buf, const uint8_t *end) {
	...
	if (buf == NULL || *buf == NULL || end == NULL || *buf > end) {
		nvtree_log("buffer is wrong");
		return (NULL);
	}
	if (*buf + sizeof(phead) > end) {
		nvtree_log("No enough size for pair header");
		return (NULL);
	}
	...
	*buf += phead.namesize;
	if (*buf > end) {
		nvtree_log("No enough size for name");
		return (NULL);
	}
	switch(phead.type) {
		case NV_TYPE_NULL: {
		...
		case NV_TYPE_BOOL: {
			...
			*buf += sizeof(b);
			if (*buf > end) {
				nvtree_log("No enough size for bool data %s", name);
				return (NULL);
			}
			break;
		}
		case NV_TYPE_NUMBER: {
			...
			*buf += sizeof(num);
			if (*buf > end) {
				nvtree_log("No enough size for number data %s", name);
				return (NULL);
			}
		...
		case NV_TYPE_STRING: {
			...
			*buf += strlen(string) + 1;
			if (*buf > end) {
				nvtree_log("No enough size for string data %s", name);
				return (NULL);
			}
			break;
		}
		...
		case NV_TYPE_DESCRIPTOR: {
			...
			*buf += sizeof(fd);
			if (*buf > end) {
				nvtree_log("No enough size for descriptor data %s", name);
				return (NULL);
			}
		...
		case NV_TYPE_NVLIST_ARRAY: {
			...
			for (uint64_t i = 0; i < phead.nitems; ++i) {
				...
				if (rp->value.tree == NULL) {
					nvtree_log("unpack nvlist array failed");
					return (NULL);
				}
				if (*buf > end) {
					nvtree_log("No enough size for nvlist data %s", name);
					return (NULL);
				}
				nvtree_add_arr(pair, rp);
			}
