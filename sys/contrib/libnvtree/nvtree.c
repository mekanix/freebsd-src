...
#define	NVTREE_FLAG_ALL_MASK	NVTREE_FLAG_BIG_ENDIAN
...
static nvtpair_t * nvtpair_unpack(uint8_t **buf, const uint8_t *end, bool isbe);

static bool
nvtree_swap_needed(bool isbe)
{
...
static uint16_t
nvtree_unpack_u16(uint16_t value, bool isbe)
{
	return (nvtree_swap_needed(isbe) ? bswap16(value) : value);
}

static uint64_t
nvtree_unpack_u64(uint64_t value, bool isbe)
{
	return (nvtree_swap_needed(isbe) ? bswap64(value) : value);
}

static int64_t
nvtree_unpack_i64(int64_t value, bool isbe)
{
	return ((int64_t)nvtree_unpack_u64((uint64_t)value, isbe));
}
	...
	if (pair->type & NVTREE_ARRAY) {
		...
		switch(pair->type) {
			...
			case NVTREE_NESTED: {
				...
				header.flags = 0;
#if BYTE_ORDER == BIG_ENDIAN
				header.flags |= NVTREE_FLAG_BIG_ENDIAN;
#endif
...
_nvtree_unpack(uint8_t **buf, const uint8_t *end) {
	...
	if ((head.flags & ~NVTREE_FLAG_ALL_MASK) != 0) {
		return (NULL);
	}
	isbe = ((head.flags & NVTREE_FLAG_BIG_ENDIAN) != 0);
	head.descriptors = nvtree_unpack_u64(head.descriptors, isbe);
	head.size = nvtree_unpack_u64(head.size, isbe);
	*buf += sizeof(head);
	root = nvtree_create();
	if (head.size == 0) {
		return (root);
	...
	while (*buf < end) {
		pair = nvtpair_unpack(buf, end, isbe);
		if (pair == NULL) {
			break;
		}
		switch (pair->type) {
...
static nvtpair_t *
nvtpair_unpack(uint8_t **buf, const uint8_t *end, bool isbe) {
	nvtpair_t *pair = NULL;
	...
	*buf += sizeof(phead);
	phead.namesize = nvtree_unpack_u16(phead.namesize, isbe);
	phead.datasize = nvtree_unpack_u64(phead.datasize, isbe);
	phead.nitems = nvtree_unpack_u64(phead.nitems, isbe);
	if (phead.namesize > 0) {
		name = (char *)(*buf);
	} else {
		name = NULL;
	...
	*buf += phead.namesize;
	...
	switch(phead.type) {
		...
		case NV_TYPE_NUMBER: {
			...
			num = nvtree_unpack_u64(num, isbe);
			*buf += sizeof(num);
			if (*buf > end) {
				return (NULL);
			}
...
nvtree_recv(const int sock) {
	...
	uint8_t dummy = 0;
	struct nvtree_header head = {};
	struct nvtree_header wire_head = {};
	struct msghdr msg = {};
	...
	if ((size_t)rc != sizeof(head) ||
	    head.magic != NVTREE_HEADER_MAGIC ||
	    head.version != NVTREE_HEADER_VERSION) {
		errno = EINVAL;
		goto err;
	}
	if ((head.flags & ~NVTREE_FLAG_ALL_MASK) != 0) {
	...
	head.descriptors = nvtree_unpack_u64(head.descriptors, isbe);
	head.size = nvtree_unpack_u64(head.size, isbe);
	buf = malloc(head.size + sizeof(head));
	memcpy(buf, &wire_head, sizeof(wire_head));
	ptr = buf;
	ptr += sizeof(head);
	size = head.size;
