...
generate_array(const char *name, size_t type)
{
	...
	for (uint64_t i = 1; i < 5; ++i) {
		switch(type) {
			case NVTREE_BOOL: {
				nvtree_add_arr(arr, nvtree_bool(NULL, true));
				break;
			}
			case NVTREE_NUMBER: {
				nvtree_add_arr(arr, nvtree_number(NULL, i));
				break;
			}
			case NVTREE_STRING: {
				nvtree_add_arr(arr, nvtree_string(NULL, "something"));
				break;
			}
			case NVTREE_NESTED: {
				nested = nvtree_nested(NULL, generate_simple_tree());
				nvtree_add_arr(arr, nested);
				break;
			}
		}
	}
