...
static constexpr uint8_t nvtpp_flag_big_endian = 0x80;
static constexpr uint8_t nvtpp_flag_all_mask = nvtpp_flag_big_endian;
...
template <typename T>
static T
unpack_data(const T &value, const bool isbe)
{
	if constexpr (!std::is_integral_v<T> || sizeof(T) == 1) {
		return value;
	} else if constexpr (sizeof(T) == 2) {
#if BYTE_ORDER == BIG_ENDIAN
		return (isbe ? value : (T)bswap16((uint16_t)value));
#else
		return (isbe ? (T)bswap16((uint16_t)value) : value);
#endif
	} else if constexpr (sizeof(T) == 4) {
#if BYTE_ORDER == BIG_ENDIAN
		return (isbe ? value : (T)bswap32((uint32_t)value));
#else
		return (isbe ? (T)bswap32((uint32_t)value) : value);
#endif
	} else if constexpr (sizeof(T) == 8) {
#if BYTE_ORDER == BIG_ENDIAN
		return (isbe ? value : (T)bswap64((uint64_t)value));
#else
		return (isbe ? (T)bswap64((uint64_t)value) : value);
#endif
	} else {
		return value;
	}
...
Tree::_unpack(void *&data, size_t &size)
{
	...
	if ((thead->flags & ~nvtpp_flag_all_mask) != 0) {
		return false;
	}
	isbe = ((thead->flags & nvtpp_flag_big_endian) != 0);

	_value.clear();
	while (size > 0) {
		offset_read(data, size, phead);
		const uint16_t namesize = unpack_data(phead->namesize, isbe);
		const uint64_t nitems = unpack_data(phead->nitems, isbe);
		if (namesize > 0) {
			name = (char *)data;
			data = ((uint8_t *)data) + namesize;
			size -= namesize;
		} else {
			name = "";
		}
		switch (phead->type) {
			...
			case NV_TYPE_NUMBER: {
				...
				offset_read(data, size, value);
				add(new Number(name, unpack_data(*value, isbe)));
				break;
			}
			case NV_TYPE_STRING: {
			...
			case NV_TYPE_NUMBER_ARRAY: {
				...
				for (uint64_t i = 0; i < nitems; ++i) {
					offset_read(data, size, value);
					arr->add(new Number("", unpack_data(*value, isbe)));
				}
				add(arr);
				break;
			}
